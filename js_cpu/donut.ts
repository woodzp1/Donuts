...
class vec3 {
    ...
    a(v: number) { return new vec3(this.x + v, this.y + v, this.z + v) }
    sub(v: vec3) { return new vec3(this.x - v.x, this.y - v.y, this.z - v.z) }
    s(v: number) { return new vec3(this.x - v, this.y - v, this.z - v) }
    mul(v: vec3) { return new vec3(this.x * v.x, this.y * v.y, this.z * v.z) }
    m(v: number) { return new vec3(this.x * v, this.y * v, this.z * v) }
    div(v: vec3) { return new vec3(this.x / v.x, this.y / v.y, this.z / v.z) }
    d(v: number) { return new vec3(this.x / v, this.y / v, this.z / v) }
    dot(v: vec3) { return this.x * v.x + this.y * v.y + this.z * v.z; }
    length() { return Math.sqrt(this.dot(this)); }
    normalize(): vec3 { const l = this.length(); return new vec3(this.x / l, this.y / l, this.z / l); }
    cross(v: vec3) {
        return new vec3(
...
function SDF(p: vec3, t: vec2) {
    let q: vec2 = new vec2(new vec2(p.x, p.z).length() - t.x, p.y);
    return q.length() - t.y;
}
...
function rayMarch(ro: vec3, rd: vec3) {
    let t = 0, d = 0;
...
function render() {
    ...
    for (let x = 0; x < width; x++) {
        for (let y = 0; y < height; y++) {
            let c = new vec2(x, y);
            let uv: vec2 = c.m(2).sub(new vec2(width, height)).div(new vec2(height, height));
            // uv = uv.m(2).s(1);
